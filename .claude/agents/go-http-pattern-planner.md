...
This is a poll→MQTT service, not a REST API, so apply the pattern **only to its
HTTP server surfaces**:

- `internal/health` — `/healthz` + `/readyz` (already a `NewServeMux` handler).
- `internal/cmd/serve.go` — the `&http.Server{}` wiring and the `runServe(ctx)`
  entrypoint (already close to `run`, but no `w`/`getenv` injection).
- `internal/mock` — the Bluelink mock API (real JSON handlers, `Handler()`
  builds the mux; the closest thing to a Ryer-style server here).

Do **not** try to force the pattern onto the outbound `bluelink` HTTP *client*,
the MQTT client, the scheduler, or the domain parsers — the pattern is about
HTTP *servers*. Call out explicitly where a pattern element does **not** apply
(e.g. `Validator`/`decode` are near-useless if there are no inbound request
bodies to validate).

## How to work

1. Read the three surfaces above and their tests before proposing anything.
