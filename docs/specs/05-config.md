...
| --- | --- | --- | --- |
| `BLUELINK_USERNAME` | — | yes | Hyundai Bluelink EU account email. |
| `BLUELINK_PASSWORD` | — | yes | Account password (RSA-encrypted before transit). |
...
| `BLUELINK_BASE_URL` | (built-in SPA host) | no | Override SPA base URL — used by tests/mock. |
| `BLUELINK_LOGIN_URL` | (built-in LOGIN host) | no | Override LOGIN host — used by tests/mock. |
...
The container image sets `TOKEN_STORE=kube` in its default env; local runs default to
`memory`. Everything else shares defaults across surfaces.
